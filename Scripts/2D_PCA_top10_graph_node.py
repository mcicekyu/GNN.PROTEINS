...
# node-level data (top-10)
X_nodes = X[:, top10].astype(float)

# per-graph means using top-10
G = int(gi.max())
...
png_out = os.path.join(OUT, "pca_side_by_side_2d.png")
plt.savefig(png_out, dpi=200)
plt.close()

# save PC scores
nodes_out = os.path.join(OUT, "PCA_nodes_top10_2d_scores.csv")
graph_out = os.path.join(OUT, "PCA_graphs_top10_2d_scores.csv")
