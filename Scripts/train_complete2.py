...
# By Adding class weights to cross_entropy if class imbalance shows up:
# class_weights = [0.8283, 1.2616]     
# Overall CV Accuracy: 0.7128 ± 0.0235
# Overall CV F1-score: 0.6937 ± 0.0229 
...
for fold, (tr_idx, te_idx) in enumerate(skf.split(np.zeros(len(labels_kept)), labels_kept), start=1):
    ...
    # create scheduler without deprecated 'verbose' arg
    ...
    # optional: log current LR (get_last_lr returns list per param group)
    current_lr = scheduler.get_last_lr()[0]
    # print initial LR once before training loop (do not reference 'epoch' before it's defined)
    print(f"  current_lr: {current_lr:.6g}")
